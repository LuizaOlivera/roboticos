...
class Env():
    ...
    def get_next_target(self):
        ...
        # Lógica para definir as coordenadas x e y do próximo alvo
        if self.initGoal:  # Defina o primeiro alvo inicialmente
            self.goal_x = 1.0
            self.goal_y = 2.0
            self.initGoal = False
        else:  # Defina os próximos alvos com base na lógica do seu ambiente
        # Exemplo: mover o próximo alvo a uma distância fixa do alvo anterior
           distance_between_targets = 1.0
           self.goal_x += distance_between_targets
           self.goal_y += distance_between_targets

        next_target.x = self.goal_x
        next_target.y = self.goal_y
